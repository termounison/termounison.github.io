...
function test(){
   ...
   */
	...
	//pruebame...
    //prob_ec_clausius_clapeyron_evaporacion();
...
//-----------------------------------------------------------------------------
function diagramaCO2(){
	
	let T0, P0, T, P, R, V1, V2, dH_fus, dH_sub, dH_eva, dP, dV;
	...
	for( T = -50; T<=30; T+=10){
		...
		P = P0*Math.exp( -(dH_eva/R)*( 1/(T+273.15) - 1/T0 ) );
				
		console.log(T, P/101325);
	}
	...
	*/
	...
	//P = P0*Math.exp( -(dH_eva/R)*( 1/T - 1/T0 ) );
	
	//P = P0*Math.exp( -(dH_sub/R)*( 1/T - 1/T0 ) );
